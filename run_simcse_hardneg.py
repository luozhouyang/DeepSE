...
from transformers_keras import DatapipeForHardNegativeSimCSE, SpearmanForSentenceEmbedding, HardNegativeSimCSE

MODEL_DIR = "models/atec-hardneg"
...
model = HardNegativeSimCSE.from_pretrained(
...
# )
train_dataset = DatapipeForHardNegativeSimCSE.from_jsonl_files(
    input_files=CONFIG["train_input_files"],
    vocab_file=VOCAB_PATH,
    sequence_key="sequence",
    pos_sequence_key="positive_sequence",
    neg_sequence_key="negative_sequence",
    batch_size=80,
...
valid_dataset = DatapipeForHardNegativeSimCSE.from_jsonl_files(
...
model.fit(
    ...
    callbacks=[
        ...
        tf.keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(os.path.join(MODEL_DIR, "hardneg-simcse-{epoch:04d}.ckpt")), save_weights_only=True
        ),
